...
describe('Add bets fifth tests', function(){
	...
	for(let i = 3 ; i < 5 ; i++) {
		it('Verify that you can place bets when not logged in.'+ i + '', function() {
			page.logout();
	...
	for(let i = 1 ; i < 4 ; i++) {
		it('Verify that you can place bets when not logged in and see the bets you placed after login.'+ i + '', function() {
			page.logout();
			page.visit('mobile/sportsbook');
			page.addBets(i);
			page.visit('');
	...
	for (let i = 1; i < 5; i++) {
		it('Verify that added bets ("' + i + '" total) are saved in the Betslip when you are navigating over the mobile website.'+ i + '', function() {
