...
describe('Next Games coupon functionality testing', function(){
	...
	it('Verify that fourth tab doesnt contain already started events', function(){
		page.nextGames().selectTab(3);
		expect(page.nextGames().dateTimes()).not.toContain(false);
		expect(page.noError()).toBe(true);
	});

	it('Verify that fifth tab doesnt contain already started events', function(){
		page.nextGames().selectTab(4);
		expect(page.nextGames().dateTimes()).not.toContain(false);
		expect(page.noError()).toBe(true);
	});
